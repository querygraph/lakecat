...
stays on the published TypeSec crate, the Sail git dependency on the
querygraph/sail `lakecat` branch remains explicit, manual workflow triggers
remain intentional, and the
...
For the already-published `v0.2.1` baseline, do not run another tag command.
For the already-published `v0.3.0` baseline, do not run another tag command.
For the already-published `v0.4.0` baseline, do not run another tag command
either — `v0.1.0` (Bobcat), `v0.2.0`/`v0.2.1` (Lynx), `v0.3.0` (Ocelot), and
`v0.4.0` (Caracal) are fixed published tags.
