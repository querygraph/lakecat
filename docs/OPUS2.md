...
warehouse-prefixed Iceberg REST namespace load/drop routes plus the in-process
Sail provider namespace drop path now resolve durable namespace state and reject
non-empty drops while tables, views, or scoped policy bindings remain.
QueryGraph bootstrap now exports those views with
