...
- **P2 — QGLake "Resilience Desk" acceptance (depends on P1).** Wire
  ...
  and the handoff summary; `lakecat-cli qglake-verify-handoff` now validates
  ...
  compact `querygraphVerification` object now embeds `verifiedTables` and
  ...
  `verified-tables` and `verified-views`; compact
  `querygraphImportVerification` now also embeds the import table/view ids,
  counts, hashes, and standards and must match both `querygraphVerification`
  and the captured QueryGraph import output; the
  ...
  namespace chain hashes, count alignment between `verifiedChainCount` and those
  chain hashes, enough receipt hashes to cover the verified chains, or
  replay/OpenLineage hashes, so the hash-chain evidence is not only enforced by
  the shell harness; the local handoff harness now includes the
...
- **P4 — Semantic catalog graph (F6).** Emit the bounded typed taxonomy
  ...
  manifest-hashed Server/Project/Warehouse tenant spine and
  ...
  graph path; saved QGLake handoff verification now also parses the archived
  bootstrap bundle artifact and reruns that same bootstrap verifier before
  accepting the compact summary; policy-binding, project, and warehouse upserts
  now also emit
...
- **P5 — Tenancy (F7) + production credentials (F8).** Project/Warehouse as
  ...
  *Started: governed server, project, and warehouse list/upsert management
  ...
  proofs plus `chain-verified` validation that now checks ordered view-version
  transitions as well as hash links: the first receipt must be a version-1
  upsert, later upserts must advance exactly one version, and tombstone drops
  must preserve the accepted durable version while linking to the previous
  receipt; lineage-drain summaries replay that verified-chain evidence for
...
- **P6 — Reproducibility (F10) + typed v4 (F9).** Land the Sail helper commits
  ...
  the concrete local Sail helper API surface, the sibling QueryGraph importer's
  `receipt-chain-hash` view evidence compatibility, and the manual-only CI
  trigger so F10 drift is executable even before automatic CI is re-enabled. The
  local Sail source state is committed on `codex/graph`; only untracked
  artifact/book directories remain outside those scoped source commits.
