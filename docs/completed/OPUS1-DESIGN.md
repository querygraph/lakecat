...
| 2 | No auth / real principal | **PARTIAL** | principal/TypeDID/bearer header resolution added with sanitized `lakecat.request-identity.v1` envelopes on authorization receipts; catalog config, namespace create/list, table create/load/commit, scan planning, task materialization, credential-vending requests, and QueryGraph bootstrap now require typed capabilities; catalog/namespace/table/scan/bootstrap/credential events and commit receipts persist in Turso audit/outbox; storage-profile modeling, a pluggable credential issuer, a TypeSec-gated secret-ref issuer path, an environment-backed `typesec://env/VARIABLE` resolver, TypeSec-authorized Vault resolution for `vault://`, and TypeSec-gated pluggable backend dispatch for `aws-sm://`, `gcp-sm://`, and `azure-kv://` refs are started; TypeDID verification has a `typesec-local` verifier seam backed by TypeSec's gateway and audit-safe attestation API, while production resolver/key configuration and remaining built-in production secret-store SDK resolver backends are still pending |
...
| 4 | No persistence backend | **PARTIAL** | Turso `TursoCatalogStore` exists for namespaces, tables, pointer log, idempotency, audit, outbox, object-write-aware commits, outbox delivery, typed inferred storage profiles, governed managed warehouse storage profiles with external secret references, a service credential-issuer hook, a TypeSec-gated secret-ref issuer path with environment-backed resolution for local runs, Vault resolution, pluggable production-provider dispatch with fail-closed defaults, governed ODRL policy bindings, and governed table soft delete/restore; built-in production secret-store SDK resolver coverage beyond Vault remains pending |
...
| 8 | Grust graph is a placeholder taxonomy | **STARTED** | The reusable LakeCat graph envelope importer and catalog-event taxonomy helper now live in Grust; LakeCat's `grust-local` sink adapts outbox events into Grust-owned event/warehouse/namespace/table graph projections, namespace creation now replays as a catalog-facing Namespace graph event, policy-binding upserts now replay as catalog-facing Policy graph events with ODRL payloads, scan planning now replays as catalog-facing ScanPlan graph events with governed read restrictions, table commits now replay as catalog-facing Commit graph events with pointer movement and idempotency hash payloads, non-anonymous principals now replay as catalog-facing Principal actor anchors, table metadata summaries now replay as catalog-facing Column and Snapshot anchors, and a boundary test verifies Grust Cypher can consume the projection. Richer graph schemas, traversal, and query behavior remain Grust work. |
| 9 | `list_namespaces` fabricates `default` | **CLOSED** | memory and Turso stores return an empty list until namespace creation |
| 10 | Side effects coupled to request path | **CLOSED** | Turso outbox, embedded memory audit/outbox parity, and service drain/projection API with receipt summaries exist; catalog handlers record events for replay and no longer emit graph/lineage inline |
...
committed outbox events into graph and lineage sinks with delivered event types
and projection counts in the drain receipt. LakeCat also has a typed
...
- **P1 — persistence + durable commit spine.** `TursoCatalogStore`;
  ...
  resolution, TypeSec-gated pluggable dispatch for AWS/GCP/Azure secret-ref
  schemes, and Turso-backed longest-prefix profile selection are started.
  Governed ODRL
  ...
  built-in external production secret-store SDK resolver coverage beyond Vault
  remains outstanding.*
...
- **P4 — typed Grust catalog graph + outbox**, then **OpenLineage + TypeDID**.
  ...
  authorization/request-identity payload; a governed lineage-drain management
  endpoint lets local acceptance flows replay the durable outbox on demand;
  TypeDID request envelopes are captured on receipts. Grust now owns reusable LakeCat graph envelope ingestion
