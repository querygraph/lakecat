...
| F7 | Single warehouse; no Project/Server/View entities | LOW | STARTED — Project/Warehouse records and warehouse-prefixed routing |
...
- **P5 — Tenancy (F7) + production credentials (F8).** Project/Warehouse as
  ...
  *Started: governed project and warehouse list/upsert management endpoints now
  ...
  routes now accept a warehouse prefix for multi-warehouse table access while
  keeping the unprefixed default-warehouse compatibility path. Server/View
  entities and richer project-scoped routing remain pending.*
