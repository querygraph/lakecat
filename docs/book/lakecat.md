...
| Governed scan paths have TypeSec-style receipt evidence | Iceberg gives table metadata and scan-planning inputs; it does not prescribe policy receipts, purpose strings, TypeDID envelopes, ODRL-derived restrictions, or agent posture. | TypeSec decides authority, LakeCat carries the same typed capability from the HTTP boundary into Sail, and binds that one receipt to tenant/table/pointer/request/audit evidence while Sail plans or validates the narrowed table work. | LakeCat/TypeSec/Sail extension today. A policy-engine-neutral proof-carrying scan profile is plausible later. |
...
That closure is shared by ordinary control-plane replay too: namespace-list
events get the same policy-binding context check as catalog config reads, so
standard Iceberg catalog navigation cannot become a weaker governance evidence
path.
...
from clean head `a186cf24`. It passed with tracked book artifact validation,
