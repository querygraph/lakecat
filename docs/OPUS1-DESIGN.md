...
- **P3 — Sail Tier 1 (`CatalogProvider`).** Start "free" via Sail's
  ...
  current-schema fields projected into Sail `TableStatus`. The next step is full
  Iceberg metadata-to-`TableStatus` conversion for nested/partition/sort details
  and planner fusion, preferably upstreamed into Sail helpers where reusable.*
