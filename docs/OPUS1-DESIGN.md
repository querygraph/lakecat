...
| 2 | No auth / real principal | **PARTIAL** | principal/TypeDID/bearer header resolution added; table load, scan planning, task materialization, and commit now require typed capabilities; commit authorization receipts persist in Turso audit/outbox; governed metadata/scan reads record durable audit/outbox events; governed vending still pending |
...
- **P2 — finish governance: capability model + governed read path.** Promote the
  ...
  Finding 2.) *Started with typed table-load, table-scan, and table-commit
  capabilities, persisted commit authorization receipts, and durable metadata-read
