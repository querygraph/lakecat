...
| 2 | No auth / real principal | **PARTIAL** | principal/TypeDID/bearer header resolution added; commit authorization receipts persist in Turso audit/outbox; table load, scan planning, and task materialization now require typed capabilities; governed metadata/scan reads record durable audit/outbox events; governed vending still pending |
...
- **P2 — finish governance: capability model + governed read path.** Promote the
  ...
  Finding 2.) *Started with typed table-load and table-scan capabilities,
  persisted commit authorization receipts, and durable metadata-read /
