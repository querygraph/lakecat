...
- Latest raw table-operation summary hardening slice:
  ...
  Raw lineage-drain summary construction now reuses the service replay
  validators for `table.commits-listed`, `table.scan-planned`, and
  `table.scan-tasks-fetched` evidence after compact-field extraction and before
  returning a summary, so otherwise valid wrapped table-history or governed
  scan payloads cannot append unverified commit-history, scan, lineage,
  QueryGraph, or application claims before compact QGLake proof inherits them.
- Local verification for this raw table-operation summary slice passed:
  `cargo test -p lakecat-service --lib lineage_drain_summary_rejects_unverified_table_operation_fields -- --test-threads=1`;
  `cargo test -p lakecat-service --lib scan_planned_drain_summary_preserves_projection_evidence -- --test-threads=1`;
  `cargo test -p lakecat-service --lib lineage_drain_summary -- --test-threads=1`;
  `cargo fmt -p lakecat-service -- --check`;
  `cargo test -p lakecat-service`;
  `docs/book/build.sh`;
  `scripts/check-release-readiness.sh --quick`; `git diff --check`.
- Latest raw view summary hardening slice:
  `Validate raw view summaries`.
  Raw lineage-drain summary construction now reuses the service replay
  validators for `view.listed`, `view.upserted`, `view.loaded`, and
  `view.dropped` evidence, rejecting malformed wrappers, view lists, view
  lifecycle scope, version guards, receipt actions, and authorization receipts
  before compact QGLake view proof can inherit them.
- Local verification for this raw view summary slice passed:
  `cargo test -p lakecat-service --lib lineage_drain_summary_rejects_malformed_view_events -- --test-threads=1`;
  `cargo test -p lakecat-service --lib lineage_drain_summary_rejects_malformed_view_versions -- --test-threads=1`;
...
- Latest implementation/testing slice:
  ...
  fields current producers emit, rejecting extra unverified table lifecycle,
  storage, graph, lineage, QGLake, QueryGraph, or application claims before
  acknowledgement, graph projection, or OpenLineage projection. A focused
  regression covers create, load, delete, and restore wrapper shapes.
- Latest implementation/testing slice:
...
- Latest implementation/testing slice:
  ...
  commit, policy, storage, graph, lineage, QueryGraph, or application claims
  before acknowledgement, graph projection, or OpenLineage projection. A
  focused regression covers table-commit wrapper drift.
- Latest implementation/testing slice:
...
- Latest implementation/testing slice:
  ...
  extra unverified credential, storage-scope, authorization, issuer, graph,
  lineage, QueryGraph, or application claims before acknowledgement, graph
  projection, or OpenLineage projection. A focused regression covers
  credential wrapper drift.
- Latest implementation/testing slice:
...
- Latest implementation/testing slice:
  ...
  `view.listed`, and management-list replay payloads over the fields current
  producers emit, rejecting extra unverified inventory, lineage, graph,
  QueryGraph, or application claims before acknowledgement, graph projection,
  or OpenLineage projection. A focused service regression covers namespace,
  view, policy, project, server, storage-profile, and warehouse list wrappers.
- Latest implementation/testing slice:
...
- Latest implementation/testing slice:
  ...
  duplicate lineage/graph outbox replay evidence.
- Local verification for this Turso audit event identity slice is green:
  `cargo test -p lakecat-store --features turso-local turso_store_duplicate_audit_write_does_not_duplicate_outbox -- --test-threads=1`
  passed; `cargo fmt -p lakecat-store -- --check` passed;
  `cargo test -p lakecat-store --features turso-local` passed;
  `scripts/check-release-readiness.sh --quick` passed;
  `docs/book/build.sh` passed;
...
- Local verification for this Turso pointer-log principal scope slice is green:
  `cargo test -p lakecat-store --features turso-local turso_store_rejects_commit_history_principal_row_drift -- --test-threads=1`
  passed; `cargo fmt -p lakecat-store -- --check` passed;
  `cargo test -p lakecat-store --features turso-local` passed;
  `scripts/check-release-readiness.sh --quick` passed;
  `docs/book/build.sh` passed;
...
- Latest implementation/testing slice:
  ...
  table-name columns before returning or mutating REST-visible table state.
- Local verification for this Turso table row-column scope slice is green:
  `cargo test -p lakecat-store --features turso-local turso_store_rejects_table_row_column_scope_drift -- --test-threads=1`
  passed;
...
- Latest implementation/testing slice:
  ...
  cannot remap a decoded active view into another warehouse/namespace/name
  scope.
- Local verification for this Turso active-view row-column scope slice is
  green:
  `cargo test -p lakecat-store --features turso-local turso_store_rejects_view_record_row_column_scope_drift -- --test-threads=1`
  passed; `cargo fmt -p lakecat-store -- --check` passed;
  `cargo test -p lakecat-store --features turso-local` passed;
  `docs/book/build.sh` passed;
  `docs/book/check_epub_metadata.sh docs/book/dist/lakecat.epub "lakecat (0.1.0)"`
...
- Local verification for this lineage-drain view receipt-chain object summary
  slice is green:
  `cargo test -p lakecat-service lineage_drain_summary_rejects_malformed_view_receipt_chain_objects -- --test-threads=1`
  passed; `cargo fmt -p lakecat-service -- --check` passed;
  `cargo test -p lakecat-service` passed; `docs/book/build.sh` passed;
  `docs/book/check_epub_metadata.sh docs/book/dist/lakecat.epub "lakecat (0.1.0)"`
  passed; `scripts/check-release-readiness.sh --quick` passed;
...
- Local verification for this lineage-drain management ID summary slice is
  green:
  ...
  `git diff --check` passed.
- Latest implementation/testing slice:
  `Harden lineage-drain fetched scan required-filter summaries`.
  Raw lineage-drain summary construction now rejects non-array
  `required-filters` evidence and fetched scan filters that drift from the
  read-restriction row predicate, so QGLake scan proof cannot silently omit or
  widen server-derived filter evidence.
- Local verification for this lineage-drain fetched scan required-filter
  summary slice is green:
...
- Local verification for this lineage-drain credential prefix summary slice is
  green:
  ...
  `git diff --check` passed.
- Latest implementation/testing slice:
  `Harden lineage-drain governed scan summaries`.
  Raw lineage-drain summary construction now rejects malformed, blank, or
  duplicate projection/stat field arrays instead of silently dropping invalid
  entries, so a future validation bypass cannot return governed scan proof
  with omitted projection or stats evidence.
- Local verification for this lineage-drain governed scan summary slice is
  green:
  `cargo test -p lakecat-service lineage_drain_summary_rejects_malformed_scan -- --test-threads=1`
  passed;
  `cargo fmt -p lakecat-service -- --check` passed;
  `cargo test -p lakecat-service` passed;
  `docs/book/build.sh` passed;
...
- Latest implementation/testing slice:
  ...
  them, so a future validation bypass cannot return QGLake replay summaries
  with omitted malformed view-history evidence.
- Local verification for this lineage-drain view receipt summary-hash slice is
  green:
  `cargo test -p lakecat-service lineage_drain_summary_rejects_malformed_view -- --test-threads=1`
  passed;
  `cargo fmt -p lakecat-service -- --check` passed;
  `cargo test -p lakecat-service` passed;
  `docs/book/build.sh` passed;
...
- Latest implementation/testing slice:
  ...
  posture, and ordered receipt hashes, so forged chain digests cannot pass
  replay admission by also appearing in the top-level `chain-hashes` array.
- Local verification for this view receipt-chain replay-hash slice is green:
...
- Local verification for this view receipt-chain replay-scope slice is green:
  `cargo test -p lakecat-service view_receipt_chain_scope_and_counts -- --test-threads=1`
  passed;
  `cargo fmt -p lakecat-service -- --check` passed;
  `cargo test -p lakecat-service` passed;
