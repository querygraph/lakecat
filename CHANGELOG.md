...
- Recorded the v0.4.0 release-candidate proof from clean head `bd5c03ba`
  (bundle `sha256:8e0c12befc66bca2…`, graph `sha256:5835bd73848046e7…`,
  OpenLineage `sha256:059d1772b1adf474…`, QueryGraph import `sha256:597db83d1646e725…`).
...
- Iceberg REST table rename: the canonical default and warehouse-prefixed
  ...
  to a destination namespace/name as one governed transaction, preserving
  metadata and version state, retargeting commit-history and table-scoped policy
  rows, retiring name-bound idempotency replays, and recording immutable
  `table.renamed` audit/outbox evidence. Graph and OpenLineage replay preserve
  both identities as a rename transformation. Rename rejects missing source or
  destination namespaces, hidden sources, occupied destinations, and
  cross-warehouse moves without partial mutation. Added wire, capability,
  multipart-identity, error, replay, shared-backend, and Turso concurrency
  coverage.
...
- Refreshed the full local release-candidate proof from clean head `6e0c3d10`.
  `scripts/check-release-readiness.sh --release-candidate` passed locally with
  the release-proof contract in clean candidate mode, the full workspace and
  feature-gate matrix, out-of-tree book artifact validation, Grust Turso QGLake
  handoff, QueryGraph locked verify/import, bundle hash
  `sha256:501efe0f6ca204301a83fe06177b9c1f97a4825e29f326c9d3dedbd58ee56fcf`,
  OpenLineage hash
  `sha256:f92d2e76e179e42feb39f775a33bf7d226f75253b1ce9a9ecc0531ddbcf45199`,
  QueryGraph import hash
  `sha256:2d8a3d009e14d6ffe0178d2355ac979e02506d2eb5ae409deeb5579c6b1ac6fa`,
  and `git diff --check`.
...
- Hardened outbox delivery admission. Memory and Turso stores now validate a
  ...
  batch delivery fails before marking any event delivered when one selected
  pending row is malformed.
...
- Added raw commit-history summary receipt-admission coverage. Raw
  ...
  evidence before compact QGLake commit-history proof can inherit pointer-log
  evidence.
...
- Extended catalog-config endpoint-set coverage to credential routes. Raw
  ...
  endpoints stay advertised beside governed plan/fetch routes.
...
- Added saved handoff verifier QueryGraph capture hash-drift coverage. The
  ...
  `artifactFiles.capturedOutputs.querygraphImport` hashes that drift from the
  compact handoff summary while keeping the outer verifier-output hash valid.
...
- Added saved handoff verifier QueryGraph captured-output manifest coverage.
  ...
  `artifactFiles.capturedOutputs.querygraphImport` hash objects even when the
...
- Added saved handoff verifier import-plan artifact manifest coverage. The
  ...
  even when the outer verifier-output hash is updated to match the tampered
  file.
...
- Added saved handoff verifier bundle artifact manifest coverage. The archived
  ...
  verifier-output hash is updated to match the tampered file.
...
- Hardened service lineage-drain fetched scan required-filter summaries. Raw
  ...
  predicate, so QGLake scan proof cannot silently omit or widen server-derived
  filter evidence.
