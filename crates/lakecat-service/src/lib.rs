...
mod tests {
    ...
    async fn outbox_drain_rejects_credential_storage_profile_id_drift_without_credentials() {
        ...
        assert!(
            message.contains(
                "credential-vend evidence storage-profile-id must match catalog evidence"
            )
        );
        assert!(message.contains("event-id-hash=sha256:"));
    ...
    async fn outbox_drain_rejects_credential_storage_profile_warehouse_drift() {
        ...
        assert!(
            message
                .contains("credential-vend storage-profile warehouse must match catalog evidence")
        );
        assert!(message.contains("event-id-hash=sha256:"));
    ...
    async fn outbox_drain_rejects_credential_secret_ref_presence_drift() {
        ...
        assert!(
            message.contains(
                "credential-vend evidence secret-ref-present must match catalog evidence"
            )
        );
        assert!(message.contains("event-id-hash=sha256:"));
    ...
    async fn outbox_drain_rejects_scan_fetch_empty_allowed_columns() {
        ...
        assert!(
            message
                .contains("scan-tasks-fetched read-restriction allowed-columns must not be empty")
        );
