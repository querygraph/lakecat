...
if [[ "$(git rev-parse "$proof_ref")" != "$(git rev-parse HEAD)" && "$candidate_mode" == "0" ]]; then
  {
    ...
    case "$changed_file" in
      CHANGELOG.md|DESIGN.md|GOAL.md|README.md|RELEASE.md|STATUS.md|docs/RELEASES.md|docs/book/lakecat.md|docs/book/dist/*|docs/completed/GOAL*.md|scripts/check-release-proof-contract.sh)
        ;;
      *)
